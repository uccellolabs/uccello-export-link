...
/* harmony export */   "UrlExportManager": () => /* binding */ UrlExportManager
/* harmony export */ });
/* harmony import */ var uccello_datatable__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! uccello-datatable */ "./node_modules/uccello-datatable/index.js");
...
var UrlExportManager = /*#__PURE__*/function () {
  function UrlExportManager() {
    var _this = this;

    _classCallCheck(this, UrlExportManager);

    this.initDatatable();
    $('#exportModal .copy-url').each(function (index, el) {
      _this.initCopyUrlButton($(el));
    });
    $('#exportModal .delete-url').each(function (index, el) {
      _this.initDeleteUrlButton($(el));
    });
  ...
  _createClass(UrlExportManager, [{
    ...
    value: function initDatatable() {
      ...
      this.datatable = new uccello_datatable__WEBPACK_IMPORTED_MODULE_0__.Datatable();
      this.datatable.init($('table[data-filter-type="list"]'));
    }
  }, {
    key: "initCopyUrlButton",
    value: function initCopyUrlButton(element) {
      var _this2 = this;

      $(element).on('click', function (event) {
        event.preventDefault();
        var inputId = element.parents('.export-url:first').find('.export-url-value').attr('id');

        _this2.copyUrl(inputId);
      });
    }
  }, {
    key: "initDeleteUrlButton",
    value: function initDeleteUrlButton(el) {
      var element = $(el);
      element.on('click', function (event) {
        event.preventDefault();
        var data = {
    ...
    key: "initGenerateExportUrlClickListener",
    value: function initGenerateExportUrlClickListener() {
      var _this3 = this;

      $('.generate-export-url').on('click', function (event) {
        event.preventDefault();
        var endpointUrl = $(event.currentTarget).attr('href');

        _this3.generateExportUrl(endpointUrl);
      });
    }
  }, {
    key: "generateExportUrl",
    value: function generateExportUrl(endpointUrl) {
      var _this4 = this;

      var formData = this.getExportConfig();
      $.post(endpointUrl, formData).then(function (response) {
        var element = $(response.html_content); // Prepend the link

        $('#exportModal #export-urls-list').prepend(element); // Initialize copy url button

        _this4.initCopyUrlButton($('.copy-url', element)); // Initialize delete url button


        _this4.initDeleteUrlButton($('.delete-url', element)); // Initialize Materialize for this added content


        var customEvent = new CustomEvent('js.init.materialize', {
          detail: {
            element: element
          }
        });
    ...
    value: function getVisibleColumns(table) {
      var visibleColumns = [];
      $('th[data-field]', table).each(function (index, element) {
    ...
    value: function getSearchConditions(table) {
      ...
      $('th[data-column]', table).each(function (index, el) {
        ...
        if (_this5.datatable.columns[fieldName].search) {
          conditions.search[fieldName] = _this5.datatable.columns[fieldName].search;
        }
      });
      return conditions;
    }
    ...
    key: "copyUrl",
    value: function copyUrl(elementId) {
...
/***/ "./node_modules/uccello-datatable/index.js":
/*!*************************************************!*\
  !*** ./node_modules/uccello-datatable/index.js ***!
  \*************************************************/
/***/ ((__unused_webpack_module, __webpack_exports__, __webpack_require__) => {
...
class Datatable {
     ...
     */
    init(element, rowAddedCallback, rowClickCallback) {
        this.table = $(element)
        this.rowAddedCallback = rowAddedCallback
    ...
    addRowToTable(record) {
        ...
        if (typeof that.rowAddedCallback !== 'undefined') {
            that.rowAddedCallback(event, that, tr, record)
        }
